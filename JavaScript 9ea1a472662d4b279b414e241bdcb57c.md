...
Variable - are space allocation to a specific amount in the ram. different variable type represent different ram usage. JavaScript is not like other programming language which is automatic parceling is done when ever we do the declaration at the moment of creating the variable. 

### String Data-Type

let space = ' ' // an empty space string
let firstName = 'Asabeneh'
let lastName = 'Yetayeh' which is considered as a character set.
...
Primitive Data type - when the space is directly allocated on the memory. 

Reference Data type - are those where the other memory location address is written on the main memory.

**operators**  

arithmetic Operator - include the default operators like (+,-,*,/,%),

logical Operator - that include the (&&, //,≠)

Rational Operator - that includes (≤ ≥ == < >)

Ternary Operator - This operator work with this sign ? which is an expression of the condition.

Assignment Operation -  is assigning a given value of one variable to other and the same is true. (”=”).
...
Method and Function and Property

Method - are pre-built function with an input and output functionality.

Function - are user defined method which is used to perform a given operation when every they are called.

Property - are method but with only output option. we use them to retrieve information about the object or class .
